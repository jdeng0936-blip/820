...
#define IMR_CON                        0x01
#define WIZ_DEFAULT_MAC                "00-E0-81-DC-53-19"
...
RT_WEAK void wiz_user_config_mac(char *mac_buf, rt_uint8_t buf_len)
{
    ...
    rt_strncpy(mac_buf, settings.mac , buf_len);
}
...
static void wiz_set_mac(void)
{
    ...
    {
        wiz_netstr_to_array(settings.mac, wiz_net_info.mac);
    }
}
...
static int wiz_network_init(rt_bool_t b_config)
{
    ...
    if(wiz_netstr_to_array(settings.self_addr, wiz_net_info.ip) != RT_EOK ||
            wiz_netstr_to_array(settings.netmask, wiz_net_info.sn) != RT_EOK ||
                wiz_netstr_to_array(settings.gw, wiz_net_info.dns) != RT_EOK ||
                    wiz_netstr_to_array(settings.gw, wiz_net_info.gw) != RT_EOK)
    {
...
static int wiz_interrupt_init(rt_base_t isr_pin)
{
    ...
    tid = rt_thread_create("wiz", wiz_data_thread_entry, RT_NULL, 1024, RT_THREAD_PRIORITY_MAX / 6, 20);
		//tid = rt_thread_create("wiz", wiz_data_thread_entry, RT_NULL, 1024, 27, 20);
    if (tid != RT_NULL)
    {
...
int wiz_init(void)
{
    ...
    wiz_interrupt_init(GET_PIN(G, 3));
