...
//	}
	...
	addr = (uint32_t)_lcd.front_buf+2*(800*sy+sx); 
...
volatile uint8_t backlight_status = 0;
...
int drv_lcd_hw_init(void)
{
    ...
    _lcd.lcd_info.framebuffer = rt_malloc_align(LCD_BUF_SIZE, 4);
    _lcd.back_buf = rt_malloc_align(LCD_BUF_SIZE, 4);
    _lcd.front_buf = rt_malloc_align(LCD_BUF_SIZE, 4);
